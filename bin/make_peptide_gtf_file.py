...
name = "SFARI"
sample_gtf = "/scratch/s/shreejoy/nxu/SFARI/work/d1/226b4ccf450d019eae905fca0d5cf0/SFARI_with_cds_refined.gtf"
reference_gtf = "/project/s/shreejoy/Genomic_references/GENCODE/gencode.v39.annotation.gtf"
peptides = "search_results/Task1SearchTask/AllPeptides.Gencode.psmtsv"
pb_gene = "/scratch/s/shreejoy/nxu/SFARI/work/76/67bf6ec1331b6789fd51141f5aa85a/SFARI_genes.tsv"
gene_isoname = "/scratch/s/shreejoy/nxu/SFARI/work/52/0e3999ec4bf6604f90166727ca88a6/gene_isoname.tsv"
refined_fasta = "/scratch/s/shreejoy/nxu/SFARI/work/aa/e1ce094de3aecc51c9ba4620671c7a/gencode_protein.fasta"
...
def main():
    ...
    sample_gtf = read_sample_gtf(sample_gtf)
    sample_pbs = process_gtf(sample_gtf)

    reference_gtf = read_reference_gtf(reference_gtf)
    ...
    gene_isoname = pd.read_table(gene_isoname, names=['gene','isoname'])
    gene_isoname = pd.Series(gene_isoname.gene.values, index=gene_isoname.isoname).to_dict()
    gene_map = {**pb_gene, **gene_isoname}
    pep = process_psmtsv(peptides, gene_map)
    seqs = read_fasta(refined_fasta)
    make_peptide_gtf(name, pbs, gene_map, pep, seqs)
