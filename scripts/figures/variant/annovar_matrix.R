...
func_to_be_replaced <- orfanage_predicted %>%
...
orfanage_predicted <- orfanage_predicted %>% 
    mutate(
        orfanage_variant_type = case_when(
            Func.refGene %in% func_to_be_replaced ~ Func.refGene,
...
bind_cols(original[, "ASD status"], gencode_predicted[, "gencode_variant_type"], orfanage_predicted[, "orfanage_variant_type"]) %>%
    ...
    mutate(
        gencode_variant_type = as.integer(factor(gencode_variant_type, levels = unique(gencode_variant_type))),
        orfanage_variant_type = as.integer(factor(orfanage_variant_type levels = unique(orfanage_variant_type)))
    )

df$region=factor(1:nrow(df))
df$total=log(df$case+df$control+1,2) ^ 0.5 / 6.5

df %>%
    ggplot() +
    geom_scatterpie(aes(x=variant_type, y=`ExonicFunc.refGene`, r=total, group=region), cols=c("case", "control")) +
