### CORRELATION EVENTS - FISHER'S EXACT TEST APPROACH ####
library(arrow)
library(dplyr)
library(ggplot2)
library(tidyr)
...
#Filter so each TSS is >= 100 counts (included + skipped) at EACH timepoint
# 1. Sum counts per event_id, timepoint, and TSS
TSS_totals <- counts_annotated %>%
  group_by(event_id, timepoint, TSS_id) %>%
  summarise(total = sum(count), .groups = "drop")

# 2. Keep only TSSs with total >=100 for ALL timepoints
TSS_keep <- TSS_totals %>%
  group_by(event_id, TSS_id) %>%
  filter(all(total >= 100)) %>% ##setting this high!
  ungroup()

# 3. Remove events with <2 TSSs left
TSS_keep_valid <- TSS_keep %>%
  group_by(event_id) %>%
  filter(n_distinct(TSS_id) >= 2) %>%
  ungroup()

# 4. Filter original counts_annotated down to valid events/TSSs
counts_filtered <- counts_annotated %>%
  semi_join(TSS_keep_valid, by = c("event_id", "TSS_id"))

# Summarise counts first
counts_grouped <- counts_filtered %>%
  group_by(timepoint, event_id, TSS_id, type) %>%
  summarise(total = sum(count), .groups = "drop") %>%
  ungroup()

counts_wide <- counts_grouped %>%
  pivot_wider(
    id_cols = c(timepoint, event_id, TSS_id),
    names_from = type,
    values_from = total,
...
#Remove the entire event_id if it's not at all3 timepoint
events_all_timepoints <- counts_filtered %>%
  distinct(timepoint, event_id) %>%
  group_by(event_id) %>%
  filter(n() == 3) %>%   # only keep events appearing in all 3 timepoints
  pull(event_id)
...
write.csv(results_list, "./code/output/Fisher_result_TSS.csv", row.names = F)

## Identify significant events:
results_list <- read.csv("./code/output/Fisher_result_TSS.csv")
