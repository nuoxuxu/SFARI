...
def build(args):
    ...
    try:
        ...
        if args.no_annotation:
            logger.info("Skipping annotation step, copying extract output to next stage")

            # Expected column headers for downstream steps
            expected_cols = [
                "seqnames", "start", "end", "name", "utr_length", "strand",
                "lastexon_cds_start", "lastexon_cds_end", "name2", "exonStarts", "exonEnds"
            ]

            # Read extract output and assign column headers
            df_extract = pd.read_csv(tf1.name, sep="\t", header=None)

            if df_extract.shape[1] != len(expected_cols):
                raise ValueError(f"Expected {len(expected_cols)} columns in extract output, got {df_extract.shape[1]}.")

            df_extract.columns = expected_cols
            df_extract.to_csv(tf2.name, sep="\t", index=False, header=True)
            tf2.close()
        else:
            # 2) Annotate 3' ends
