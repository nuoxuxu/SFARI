...
transcripts <- ORFanage_replaced[ORFanage_replaced$type == "transcript"] #158,844

#Only use CAGE verified tr
CAGE_ver <- classification %>% filter(within_CAGE_peak) %>% pull(isoform) #155,888
transcripts <- transcripts[transcripts$transcript_id %in% CAGE_ver] #139,778
...
transcript_df <- transcript_df %>%
  mutate(transcript_start = if_else(strand == "+", start, end)) %>%
  dplyr::select(transcript_id, gene_id, transcript_start, strand)


cluster_transcript_starts <- function(df, window_size = 100) { #100 nt for TSSs
  df %>%
    mutate(
      transcript_start = as.numeric(transcript_start),
      strand = as.character(strand)
    ) %>%
    arrange(gene_id, strand, transcript_start * ifelse(strand == "+", 1, -1)) %>%
    ...
    mutate(cluster_id = {
      ...
      if (n_trans == 1) {
        ...
        start_pos <- transcript_start[1]
        
        for (i in 2:n_trans) {
          dist <- if (strand[1] == "+") {
            transcript_start[i] - start_pos
          } else {
            start_pos - transcript_start[i]
          }
    ...
    mutate(
      cluster_proximal_coord = if (cur_group()$strand == "+") {
        max(transcript_start, na.rm = TRUE) 
      } else {
        min(transcript_start, na.rm = TRUE) 
      }
    ) %>%
    ungroup()
}
...
clustered_df <- cluster_transcript_starts(transcript_df)

#Remove transcripts with only 1 TSS site:
filtered_df <- clustered_df %>%
  group_by(gene_id) %>%
  filter(n_distinct(cluster_id) > 1) %>%
  ungroup()
filtered_tr <- unique(filtered_df$transcript_id) #90,375
...
filtered_df_ES <- filtered_df %>%
  filter(transcript_id %in% all_transcripts) #77,453

#Re-do the filtering to remove genes w/ only 1 TSS site
...
write.csv(filtered_df_ES, "./code/AS_APA/input/transcript_TSS_sites.csv", 
          row.names = F)
...
# now we have: transcript_id | timepoint | count | TSS_id

# ExTSSnd total_transcripts to long
tot_map <- ES_events %>%
...
# columns: transcript_id | timepoint | count | TSS_id | event_id | type
write.csv(counts_annotated, "./code/AS_APA/input/tr_TSS_counts.csv", row.names = F)


# 1. Sum counts per event_id, timepoint, and TSS
TSS_totals <- counts_annotated %>%
  group_by(event_id, type, TSS_id, timepoint) %>%
  summarise(total = sum(count), .groups = "drop") 

test2 <- TSS_totals
test2$gene_id <- sub(";.*", "", test2$event_id)
#4355 genes

write.csv(TSS_totals, "./code/AS_APA/input/TSS_counts.csv", row.names = F)
