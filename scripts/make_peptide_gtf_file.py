...
def write_peptide_gtf(name, pep_ranges, pbs):
...
seqs = read_fasta("full_AA.fasta")

pb_gene = SingleCell("results/long_read/pbid_filtered.h5ad").var["pbid", "associated_gene"]
...
percolator_res = pl.read_csv("results/long_read/percolator_2.tsv", has_header=True, separator="\t")\
    .with_columns(
       ...
       num_prots = pl.col("proteinIds").str.split(",").map_elements(lambda x: len(x), return_dtype=pl.Int64)
    )\
    .explode("proteinIds")\
    .filter(
        pl.col("q-value") < 0.05,
    ...
    .with_columns(
        pl.col("peptide").str.replace(r"M\[15.9949\]", "M")
    )\
    .with_columns(
    ...
    .join(pb_gene.rename({"pbid": "pb_acc"}), on = "pb_acc", how = "left")\
    .rename({"associated_gene": "gene"})[['pep', 'pb_acc', 'prev_aa','next_aa', 'gene', 'num_prots']]
...
pbs = process_gtf(read_sample_gtf("nextflow_results/SFARI_with_cds_refined.gtf"))

write_peptide_gtf("SFARI", pep_ranges.to_pandas(), pbs)

transcript_id_to_remove = read_gtf("SFARI_peptides.gtf")\
    .group_by("transcript_id")\
    .agg(
        pl.col("strand").unique().map_elements(lambda x: len(x))
    )\
    .filter(
        pl.col("strand")==2
    )["transcript_id"]

read_gtf("SFARI_peptides.gtf")\
    .filter(
        pl.col("transcript_id").is_in(transcript_id_to_remove).not_()
    )\
    .drop("transcript_id")\
    .write_csv("SFARI_peptides_updated.gtf", separator = "\t", quote_style="never", include_header=False)