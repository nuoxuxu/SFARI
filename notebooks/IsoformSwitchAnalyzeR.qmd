...
IsoseqsSwitchList <- analyzeORF(IsoseqsSwitchList, bsg)
```

```{r}
#| label: extractSequence
library("BSgenome.Hsapiens.UCSC.hg38")
IsoseqsSwitchList <- extractSequence(IsoseqsSwitchList, bsg)
```
...
```{bash}
#| label: full-analyzeAlternativeSplicing
...
unique(IsoseqsSwitchList$isoformFeatures$isoform_id) %>%
    writeLines("pbid_list.txt")
```

```{bash}
#| label: seqkit
seqkit grep -f pbid_list.txt SQANTI3_qc_corrected.fasta > cpc2.fasta
```
...
cd libsvm-3.18
make clean && make
cd $CPC_HOME && python ./bin/CPC2.py -i /scratch/s/shreejoy/nxu/SFARI/cpc2.fasta -o /scratch/s/shreejoy/nxu/SFARI/CPC2result.txt
```
...
# Running Pfam_scan
cd ${SCRATCH}/SFARI
/home/s/shreejoy/nxu/tools/PfamScan/pfam_scan.pl -fasta isoformSwitchAnalyzeR_isoform_AA.fasta -dir /home/s/shreejoy/nxu/tools/Pfam_flat_files -cpu 40 -outfile pfam_scan_results.txt
...
IsoseqsSwitchList <- readRDS("results/long_read/IsoseqsSwitchList_part1_Analyzed_2.rds")

```

```{r}
IsoseqsSwitchList_part2 <- readRDS("IsoseqsSwitchList_part2.rds")
```
...
extractConsequenceEnrichment(
    IsoseqsSwitchList_part1_Analyzed_2,
...
```{r}
analyzeSwitchConsequences(IsoseqsSwitchList)
```
...
```{r}
extractConsequenceEnrichment(IsoseqsSwitchList)
```
