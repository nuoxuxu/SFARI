...
pl.concat(
    [
        pl.DataFrame(((lr_bulk.X.T / lr_bulk.X.sum(axis=1)) * 1e6).toarray().T,schema=lr_bulk.var_names.to_list()), lr_bulk.obs_names.to_frame()
    ],
    how="horizontal"
    )\
    .with_columns(
        sampleID = pl.col("sampleID").map_elements(lambda s: s.split("_")[0], return_dtype=pl.String)
    )\
    .group_by("sampleID")\
    .agg(pl.col("*").mean())\
    .transpose(column_names="sampleID", include_header=True, header_name="pbid")\
    .join(lr_bulk.var["pbid", "associated_transcript"], on="pbid", how="left")\
    .rename({"pbid": "transcript_id"})\
    .with_columns(
        transcript_id = pl.when(pl.col("associated_transcript")=="novel").then(pl.col("transcript_id")).otherwise(pl.col("associated_transcript"))
    ).drop("associated_transcript")\
    .join(PBID_to_TALON_ID, on="transcript_id", how="left")\
    .with_columns(
...
pl.read_csv("/Users/xunuo/projects/Dev_Brain_IsoSeq/data/cp_vz_0.75_min_7_recovery_talon_abundance_filtered.tsv", separator="\t")\
    ...
    .group_by("transcript_id").sum()\
    .with_columns(
        cs.ends_with("CP") / cs.ends_with("CP").sum() * 1e6,
        cs.ends_with("VZ") / cs.ends_with("VZ").sum() * 1e6
    )\
    .select(
