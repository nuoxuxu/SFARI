...
CDS_gtf.write_csv("transcript_vis_app/genome_gff3_gtf.csv")
```
...
```{python}
#| label: import-SFARI_peptides
SFARI_peptides = read_gtf("nextflow_results/SFARI_peptides.gtf", attributes = ["gene_name", "transcript_id", "mapped_to_novel_transcripts_uniquely"])
```


```{python}
SFARI_peptides\
    ["seqname", "start", "end", "strand", "gene_name", "transcript_id", "mapped_to_novel_transcripts_uniquely"]\
    .write_csv("transcript_vis_app/SFARI_peptides.csv")
```