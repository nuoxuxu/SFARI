...
# Add ORF coordinates to classification.txt

```{python}
transdecoder_bed = pl.read_csv("full_nt.fasta.transdecoder.bed", separator="\t", comment_prefix="track name=", new_columns=["chrom", "chromStart", "chromEnd", "name", "score", "strand", "thickStart", "thickEnd", "itemRgb", "blockCount", "blockSizes", "blockStarts"])\
    .with_columns(
...
Add TransDecoder predictions (CDS_start, CDS_end, coding, CDS_length) and GENCODE annotation (transcript_type) to lr_bulk.var

```{python}
lr_bulk.var = lr_bulk.var\
    .join(transdecoder_bed.rename({"chrom": "pbid"})["pbid", "thickStart", "thickEnd", "ORF_type", "is_uniprot"], how="left", on="pbid")\
    .with_columns(
        CDS_start = pl.col("thickStart"),
        CDS_end = pl.col("thickEnd"),
        coding = pl.when((pl.col("ORF_type")=="complete") & (pl.col("is_uniprot"))).then(True).otherwise(False)
    )\
    .with_columns(
        CDS_length = pl.col("CDS_end") - pl.col("CDS_start")
    ).drop(["thickStart", "thickEnd", "ORF_type"])\
...
lr_bulk.var\
    .filter(
        pl.col("coding"),
        pl.col("structural_category").is_in(["full-splice_match", "incomplete-splice_match"])
    )\
